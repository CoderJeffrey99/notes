...
/*
 ...
 1).特点：均以"#"开头/不要“;”/编译之前就会执行
 2).包含：文件包含、宏定义、条件编译
 3).代码执行过程：源文件 -> 预处理 -> 汇编 -> 二进制 -> 可执行文件
 */
// 1>.文件包含：将stdio.h文件copy到当前文件中
// 不用必须写在文件最上面
// 均以"#"开头/不要";"
// <>表示先从开发工具的编译器环境中查找，如果找不到再到系统的编译环境中查找
// ""表示先从当前文件所在的文件夹中查找，然后再从开发工具的编译器环境中查找，如果找不到再到系统的编译环境中查找
#include <stdio.h>
// 2>.宏定义：编译预处理时将源代码中"宏名"替换成"宏内容"/不需要"="/不需要“;”/作用域：从定义那行开始一直到文件末尾
// 系统不会为宏分配内存
// 1).命名规则：1.全部大写|2.k+驼峰命名
// 2).不带参数的宏
#define PI 3.14
// 提前结束宏定义的作用域
//#undef PI
...
/*
 ...
 我也不能去释放他，因为我不能确定，别人在别的地方是否还需要使用这块内存。内存泄露在所难免了
 */
