...
public class CMGameProxy { // 类名尽量不要相同
    /**
     * 6.main函数：程序的入口（凡是需要加载的类全部加载到JVM中才会执行main方法）
    ...
    private void showPackageBase() {
         ...
         * windows+D/R/L/E - 快速显示桌面/打开'运行'程序/切换用户/启动'我的电脑'程序
         * 2>.人机交互
        ...
        // 运行阶段：源代码不参与程序的运行过程（编译成字节码文件xxx.class以后可以直接删除java源代码xxx.java）
         ...
         * 1kb=1024byte   1Mb=1024kb   1Gb=1024Mb   1Tb=1024Gb/计算机分配内存的最小单位是字节byte
...
//        char a9 = '12'; // 不能放置两个字符
    ...
    private void showPackageOperator(CMGameProxy gameProxy) {
        // 1>.算术运算符+ - * / %
...
//        a27 = a19 ^ a20; // 两边结果相同，结果为假；两边结果不同，结果为真
        ...
        // 8>.对象运算符
        if (gameProxy instanceof CMGameProxy) {
...
//        System.out.println(bigDecimal_01.subtract(bigDecimal_02));
        ...
        r1.nextFloat(); // 0.0 ~ 1.0之间均匀分布的float值
        r1.nextDouble(); // 0.0 ~ 1.0之间均匀分布的double值
    ...
    private void showPackageScanner() {
        ...
        String s1 = scanner.next(); // 接收一个字符串：遇到空格就会结束（实际操作中如果多输入一个空格就会导致数据错误）
        String s2 = scanner.nextLine(); // 接收一个字符串：遇到回车才会结束
        scanner.close(); // 关闭扫描器
        /**
         * 面试题
...
//        // static关键字：主要用来修饰类的内部成员（成员变量、成员方法、语句块、内部类）
...
//        System.out.println("hello world");
    ...
    public void showPackageFiles() {
        ...
        if (file.delete()) {
            // 删除File对象表示的文件或目录（如果表示的是文件夹则该文件夹必须为空才可以删除）
        }
